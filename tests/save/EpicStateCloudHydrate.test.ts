...
// `epicancer_state` blob (an allowlisted payload key), so the
...
/** A CG-cloud snapshot whose `epicancer_state` blob carries a full set of
...
describe('epicancer_state cloud hydrate → composable refresh', () => {
...
describe('epicancer_state persistence round-trip (write → sdk.data)', () => {
