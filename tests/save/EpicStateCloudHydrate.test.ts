...
// `volt-flicker_state` blob (an allowlisted payload key), so the
...
/** A CG-cloud snapshot whose `volt-flicker_state` blob carries a full set of
...
describe('volt-flicker_state cloud hydrate → composable refresh', () => {
...
describe('volt-flicker_state persistence round-trip (write → sdk.data)', () => {
